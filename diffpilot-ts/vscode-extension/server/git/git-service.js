 ...
 */
export async function runGitCommand(args, workingDirectory, timeoutMs = DEFAULT_TIMEOUT_MS) {
    try {
 ...
 */
export async function findBaseBranch(workingDirectory, currentBranch, remote = 'origin') {
 ...
 */
async function findFromReflog(workingDirectory, currentBranch) {
    const result = await runGitCommand(`reflog show ${currentBranch} --format=%gs`, workingDirectory);
    if (result.exitCode !== 0 || !result.output.trim()) {
        return null;
    }
 ...
 */
async function findFromTrackingConfig(workingDirectory, branch) {
    const result = await runGitCommand(`config --get branch.${branch}.merge`, workingDirectory);
    if (result.exitCode !== 0 || !result.output.trim()) {
        return null;
    }
 ...
 */
async function getBranchRemote(workingDirectory, branch) {
    const result = await runGitCommand(`config --get branch.${branch}.remote`, workingDirectory);
    return result.exitCode === 0 ? result.output.trim() : null;
}
/**
 * Finds which remote contains the specified branch.
 */
async function findRemoteForBranch(workingDirectory, branch) {
    // First check config
    ...
    for (const remoteName of remotesResult.output.split('\n').filter(Boolean)) {
        const checkResult = await runGitCommand(`show-ref --verify --quiet refs/remotes/${remoteName.trim()}/${branch}`, workingDirectory);
 ...
 */
async function findFromUniqueMergeBase(workingDirectory, currentBranch, remote) {
    ...
    const currentHeadResult = await runGitCommand(`rev-parse ${currentBranch}`, workingDirectory);
    if (currentHeadResult.exitCode !== 0) {
        return null;
    }
    const currentHead = currentHeadResult.output.trim();
    let uniqueBase = null;
    let uniqueRemote = null;
    ...
    for (const branch of localBranches) {
        ...
        if (candidateCount === 1) {
            ...
            const isNewCandidateChildOfPrevious = await isBranchAhead(workingDirectory, branch, uniqueBase);
            if (isNewCandidateChildOfPrevious) {
                ...
                const isPreviousChildOfNewCandidate = await isBranchAhead(workingDirectory, uniqueBase, branch);
    ...
    for (const branch of remoteBranches) {
        ...
        if (candidateCount === 1) {
            ...
            const isNewCandidateChildOfPrevious = await isBranchAhead(workingDirectory, remoteRef, remoteUniqueRef);
            if (isNewCandidateChildOfPrevious) {
                uniqueBase = branch;
                uniqueRemote = remote;
                candidateCount = 1;
                ...
                const isPreviousChildOfNewCandidate = await isBranchAhead(workingDirectory, remoteUniqueRef, remoteRef);
                if (isPreviousChildOfNewCandidate) {
                    candidateCount = 1;
                }
            }
 ...
 */
async function getMergeBase(workingDirectory, branch1, branch2) {
 ...
 */
async function isBranchAhead(workingDirectory, branch, baseBranch) {
    const result = await runGitCommand(`rev-list --count ${baseBranch}..${branch}`, workingDirectory);
 ...
 */
async function getLocalBranches(workingDirectory, excludeBranch) {
    const result = await runGitCommand('branch --list --format=%(refname:short)', workingDirectory);
    if (result.exitCode !== 0)
        return [];
 ...
 */
async function getRemoteBranches(workingDirectory, remote) {
    const result = await runGitCommand(`branch -r --list "${remote}/*" --format=%(refname:short)`, workingDirectory);
    if (result.exitCode !== 0)
        return [];
    const prefix = `${remote}/`;
    return result.output
