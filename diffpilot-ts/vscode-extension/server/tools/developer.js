 ...
 */
import { runGitCommand, getCurrentBranch, findBaseBranch, getWorkingDirectory, } from '../git/git-service.js';
import { success, error, gitError, truncateContent } from './types.js';
/**
 ...
 */
function determineCommitType(analysis, diff) {
    const lowerDiff = diff.toLowerCase();
    if (lowerDiff.includes('test') || lowerDiff.includes('spec'))
        return 'test';
    if (lowerDiff.includes('.md') || lowerDiff.includes('readme') || lowerDiff.includes('doc'))
        return 'docs';
    if (lowerDiff.includes('config') || lowerDiff.includes('.json') || lowerDiff.includes('.yaml'))
        return 'chore';
...
export async function generateCommitMessage(args) {
    ...
    if (hasStaged) {
        ...
        if (unstagedResult.exitCode !== 0 || !unstagedResult.output.trim()) {
            return error('No changes found. Please stage your changes with `git add` or make some modifications first.');
        }
 ...
 */
function scanForSecrets(diff, source) {
    const findings = [];
 ...
 */
export async function scanSecrets(args) {
    const repoDir = getWorkingDirectory();
...
function parseNumstat(numstatOutput) {
    const stats = { added: 0, removed: 0, files: 0, fileDetails: [] };
 ...
 */
export async function diffStats(args) {
    const repoDir = getWorkingDirectory();
    ...
    if (baseBranch || featureBranch) {
        ...
        const branchStat = await runGitCommand(`diff --numstat origin/${baseBranch}...${featureBranch}`, repoDir);
        const branchStats = parseNumstat(branchStat.output);
        // Get commit count
        const commitCount = await runGitCommand(`rev-list --count origin/${baseBranch}..${featureBranch}`, repoDir);
        output += '| Metric | Value |\n|--------|-------|\n';
 ...
 */
function analyzeCodeForTests(diff) {
    const analysis = { changedFiles: [] };
    let currentFile = {
        name: 'unknown',
        isTestFile: false,
        isConfig: false,
    ...
    for (const line of diff.split('\n')) {
        ...
        if (content.includes('== null') || content.includes('!= null') || content.includes('?? ') || content.includes('?.'))
            currentFile.hasNullChecks = true;
        if (content.includes('if ') || content.includes('switch ') || content.includes('? '))
            currentFile.hasConditionalLogic = true;
        if (content.includes('for ') || content.includes('foreach ') || content.includes('while '))
            currentFile.hasLoops = true;
        if (content.includes('DbContext') || content.includes('SqlConnection') || content.includes('Repository'))
            currentFile.hasDatabaseCalls = true;
        if (content.includes('HttpClient') || content.includes('fetch(') || content.includes('axios'))
 ...
 */
export async function suggestTests(args) {
    const repoDir = getWorkingDirectory();
    let diffOutput;
    let context;
    if (args?.baseBranch) {
 ...
 */
export async function generateChangelog(args) {
    const repoDir = getWorkingDirectory();
    ...
    await runGitCommand('fetch origin', repoDir);
    // Get commit log
    const logResult = await runGitCommand(`log origin/${baseBranch}..${featureBranch} --pretty=format:"%h|%s|%an|%ad" --date=short`, repoDir);
