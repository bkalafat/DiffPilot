 ...
 */
import { runGitCommand, getCurrentBranch, findBaseBranch, isValidBranchName, getWorkingDirectory, } from '../git/git-service.js';
 ...
 */
async function extractBranchParameters(args, repoDir) {
    ...
    if (!featureBranch) {
        ...
        if (!featureBranch) {
            return {
                ...
                featureBranch: '',
                remote,
                error: gitError('Failed to detect current branch', 'Could not determine current branch. You may be in a detached HEAD state.'),
            };
        }
    }
    ...
    if (!baseBranch) {
        ...
        if (baseInfo) {
            ...
            return {
                ...
                featureBranch: '',
                remote,
                error: error(`Could not automatically determine the base branch for '${featureBranch}'. ` +
 ...
 */
async function getDiff(repoDir, remote, baseBranch, featureBranch) {
 ...
 */
export async function getPrDiff(args) {
    ...
    // Return diff content
    const header = `## Diff: ${remote}/${baseBranch} → ${featureBranch}\n\n` +
 ...
 */
export async function reviewPrChanges(args) {
    const repoDir = getWorkingDirectory();
    // Extract and validate parameters
    const branches = await extractBranchParameters(args, repoDir);
    if (branches.error) {
    ...
    const statsResult = await runGitCommand(`diff --stat ${remote}/${baseBranch}...${featureBranch}`, repoDir);
    // Build review prompt
 ...
 */
export async function generatePrTitle(args) {
    const repoDir = getWorkingDirectory();
    // Extract and validate parameters
    const branches = await extractBranchParameters(args, repoDir);
    if (branches.error) {
    ...
    const statsResult = await runGitCommand(`diff --stat ${remote}/${baseBranch}...${featureBranch}`, repoDir);
    const logResult = await runGitCommand(`log --oneline ${remote}/${baseBranch}..${featureBranch}`, repoDir);
 ...
 */
export async function generatePrDescription(args) {
    const repoDir = getWorkingDirectory();
    // Extract and validate parameters
    const branches = await extractBranchParameters(args, repoDir);
    if (branches.error) {
