...
import { checkRateLimit, sanitizeOutput, sanitizeErrorMessage, logSecurityEvent, } from './security/security-helpers.js';
// Import PR review tools
import { getPrDiff, reviewPrChanges, generatePrTitle, generatePrDescription, } from './tools/pr-review.js';
// Import developer tools
import { generateCommitMessage, scanSecrets, diffStats, suggestTests, generateChangelog, } from './tools/developer.js';
// ============================================================================
// Tool Definitions
// ============================================================================
const TOOLS = [
        ...
        try {
            ...
            switch (name) {
                ...
                case 'review_pr_changes':
                    result = await reviewPrChanges(args);
                    break;
                case 'generate_pr_title':
                    result = await generatePrTitle(args);
                    break;
                case 'generate_pr_description':
                    result = await generatePrDescription(args);
                    break;
                // Developer Tools
                case 'generate_commit_message':
                    result = await generateCommitMessage(args);
                    break;
                case 'scan_secrets':
                    result = await scanSecrets(args);
                    break;
                case 'diff_stats':
                    result = await diffStats(args);
                    break;
                case 'suggest_tests':
                    result = await suggestTests(args);
                    break;
                case 'generate_changelog':
                    result = await generateChangelog(args);
                    break;
                default:
                    return {
